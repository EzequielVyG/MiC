...
const EditEvento = () => {
	...
	const onSubmit = async (values: Record<string, any>) => {
		try {
			...
			if (isDraft) {
				router.push({
					pathname: `/mi_perfil`,
					query: { message: response.message, tab: 2 },
				});
			}
	...
	const handleConfirmModal = async () => {
		...
		router.push({
			pathname: `/mi_perfil`,
			query: { message: `Evento ${action} con éxito` },
		});
	};
	...
	return (
		<MainLayout>
			{isLoading ? (
				...
				<BasicLayout title={id === 'new' ? t['createEvent'] : t['editEvent']}>
					<Formik
						enableReinitialize
								...
								<div style={{ margin: '0px 30px 0px 30px' }}>
									<div style={inputStyle}>
										<Grid
											...
											justifyContent={'center'}
										>
											<Label text={t['selectLanguage']}></Label>
											<Select
									...
									<div style={{ margin: '0px 30px 0px 30px' }}>
										...
										<Snackbar
											...
											message={<Typography>{t['snackbarPhoto']}</Typography>}
										/>
										<br />
										<br />
										<FileUploadPreview
